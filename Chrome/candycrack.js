CandyCrack = function () {
    ...
    self.getScore = function () {
        if(self.mode == self.RANDOM) {
            $('#score').val(parseInt((1 + Math.random()) * 400000));
            endGame();
        }
        else if (self.mode == self.RATING)
            getScoreByRating();
...
$(document).ready(function () {
    ...
    $('#completeLevelButton').click(function () {
        candyCrack.counter = 0;
        candyCrack.maxCounter = parseInt($('#iterations').val());
        candyCrack.hack();
    });
